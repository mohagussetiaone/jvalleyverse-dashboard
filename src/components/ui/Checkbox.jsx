...
const Checkbox = ({ id, disabled, label, value, name, onChange, activeClass = "ring-black-500  bg-slate-900 dark:bg-slate-700 dark:ring-slate-700 " }) => {
  return (
    <label className={`flex items-center ${disabled ? " cursor-not-allowed opacity-50" : "cursor-pointer"}`} id={id}>
      ...
      <span className="text-slate-500 ml-2 dark:text-slate-400 text-sm leading-6 capitalize">{label}</span>
    </label>
  );
};
